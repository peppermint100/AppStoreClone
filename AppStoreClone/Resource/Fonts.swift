//
//  Fonts.swift
//  AppStoreClone
//
//  Created by peppermint100 on 3/30/24.
//

import UIKit

enum Fonts {
    static let title = UIFont.systemFont(ofSize: 28, weight: .bold)
    static let subtitle = UIFont.systemFont(ofSize: 18, weight: .semibold)
    
    static let headerTitle = UIFont.systemFont(ofSize: 22, weight: .bold)
    static let headerSubtitle = UIFont.systemFont(ofSize: 18, weight: .semibold)
    
    static let appDetailTitle = UIFont.systemFont(ofSize: 22, weight: .semibold)
    static let appDetailSubtitle = UIFont.systemFont(ofSize: 14, weight: .regular)
    
    static let listTitle = UIFont.systemFont(ofSize: 16, weight: .bold)
    static let listSubtitle = UIFont.systemFont(ofSize: 12, weight: .bold)
    
    static let list = UIFont.systemFont(ofSize: 16, weight: .regular)
    static let footnote = UIFont.systemFont(ofSize:10, weight: .regular)
    static let caption = UIFont.systemFont(ofSize: 14, weight: .regular)
    static let strongCaption = UIFont.systemFont(ofSize: 14, weight: .bold)
    static let strong = UIFont.systemFont(ofSize: 22, weight: .black)
}
